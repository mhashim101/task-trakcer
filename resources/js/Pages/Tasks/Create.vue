...
const props = defineProps({
  teams: Object, // or Array if it's multiple
  members: Array,
  tasks: Array
})
...
// Submit handler
const submit = () => {
  router.post('/team-lead/tasks', form, {
    onSuccess: () => {
      Object.keys(form).forEach(key => {
        form[key] = '' // Reset form fields
      })
    }
  })
}
        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
          <form @submit.prevent="submit">
            ...
            <div class="mb-4">
              ...
              <textarea v-model="form.description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required></textarea>
              <div v-if="form.errors?.description" class="text-red-500 text-sm mt-1">{{ form.errors.description }}</div>
            </div>

            <div class="mb-4">
              <label for="team_id" class="block text-sm font-medium text-gray-700">Team</label>
              <select v-model="form.team_id" id="team_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                <option :value="props.teams.id">{{ props.teams.name }}</option>
              </select>
            </div>
            <!-- {{ $page.props.teams.id }} -->
            ...
            <div class="mb-4">
              ...
              <select v-model="form.assigned_to" id="assigned_to" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                <option v-for="member in props.members" :key="member.id" :value="member.id">{{ member.name }}</option>
              </select>
            </div>
