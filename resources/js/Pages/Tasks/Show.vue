...
const formatDate = (date) => {
  return date ? new Date(date).toISOString().split('T')[0] : 'Not set'
}
</script>
        ...
        <div class="p-6 bg-white border-b border-gray-200">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <p>{{ task.team?.name || 'No Team Assigned' }}</p>
            </div>

            <div>
              <h4 class="text-md font-medium mb-2">Assigned By</h4>
              <p>{{ task.assigned_by?.name || 'Unknown' }}</p>
            </div>

            <div>
              <h4 class="text-md font-medium mb-2">Assigned To</h4>
              <p>{{ task.assigned_to?.name || 'Unassigned' }}</p>
            </div>
          ...
          <div class="flex items-center justify-end space-x-4">
            ...
            <Link href="/team-lead/tasks" class="bg-gray-500 text-white px-4 py-2 rounded">Back</Link>
          </div>
        </div>
      </div>
    </div>
